  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-950 to-black p-4 font-sans antialiased">
      <div className="bg-gray-800 p-8 rounded-2xl shadow-xl w-full max-w-md border border-gray-700">
        <h1 className="text-4xl font-extrabold text-white mb-6 text-center">Junte-se à <span className="text-purple-600">Zenith</span> </h1>
        <p className="text-gray-400 text-center mb-8">  Crie sua conta e comece sua jornada agora mesmo. </p>

        <form onSubmit={handleCadastro} className="space-y-4">
          <div className="relative">
            <Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
            <input type="email" placeholder="E-mail" value={email} onChange={(e) => setEmail(e.target.value)} 
            className="w-full p-3 pl-10 rounded-lg bg-gray-700 text-white border border-gray-700 focus:border-purple-600 focus:ring-2 focus:ring-purple-600 focus:ring-opacity-50 transition duration-200" 
        ...
        <div className="relative my-8">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-2 bg-gray-800 text-gray-400">Ou entre com</span>
          </div>
        </div>

        <div className="space-y-4">
          <button onClick={() => handleSocialLogin("google")} className="w-full bg-red-600 hover:bg-red-500 active:bg-red-700 cursor-pointer text-white font-bold py-3 px-4 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-600 focus:ring-offset-2 focus:ring-offset-gray-800 transition duration-300 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg"
           disabled={loading} >
            {loading ? (
          ...
          <button onClick={() => handleSocialLogin("github")} className="w-full bg-gray-700 hover:bg-gray-600 active:bg-gray-900 cursor-pointer text-white font-bold py-3 px-4 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-600 focus:ring-offset-2 focus:ring-offset-gray-800 transition duration-300 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg" disabled={loading} >
            {loading ? (
              <div className="animate-spin rounded-full h-5 w-5 border-t-2 border-b-2 border-white"></div>
            ) : (
              <>
        ...
        <p className="text-center text-gray-400 text-sm mt-8">
          Já tem uma conta?{' '}
          <Link href="/auth/login" className="text-purple-500 cursor-pointer hover:text-purple-400 font-bold focus:outline-none flex items-center justify-center gap-1 mx-auto mt-2">
